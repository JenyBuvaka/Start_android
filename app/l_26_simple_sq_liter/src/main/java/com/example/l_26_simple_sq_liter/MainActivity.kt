package com.example.l_26_simple_sq_liter
...
//функция
        ...
        btnFunc.setOnClickListener {
            if (!TextUtils.isEmpty(etFunc.text)) {
                ...
                c.use { worckCursor(c) }
            }else c = db.query("mytable",null,null,null,null,null,null)
        }
...
//Насиление по региону
        ...
        btnHaving.setOnClickListener {
            val etRegionStr = etRegionPeople.text.toString()

            if(!TextUtils.isEmpty(etRegionPeople.text)) {
                Log.d(
                ...
                c.use { worckCursor(c) }
                if (etRegionStr.toDouble()>1400){
                    tv.text = "По вашему запросу ничего не найдено"
                }
            }else{
                c = db.query("mytable", null, null, null, null, null, null, null)
            tv.text = "Введите допустимое значение "}
        }
        //СОРТИРОВКА
    ...
    fun worckCursor(c: Cursor){
        val stringBuilder = StringBuilder()// Используйте StringBuilder для конкатенации строк
        if (c.moveToFirst()) {
            var str: String
            do {
                str = ""
                ...
                stringBuilder.append(str).append("\n")// Добавляем текущую строку к общей строке
            } while (c.moveToNext())

        }
        c.close()
        tv.text = stringBuilder.toString()// Устанавливаем общую строку как текст в TextView
    }
}
